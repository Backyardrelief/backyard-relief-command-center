...
export default function DriverDashboard() {
  ...
  const sendText = async (
    ...
    if (error) {
      console.error(
        "SMS function invocation failed:",
        error
      );
    ...
    if (!data) {
      return {
        ...
        duplicate: false,
        reason:
          "The SMS function returned no response.",
      };
    }
  ...
  const handleOnTheWay = async (
    ...
    if (textResult.sent === true) {
      showMessage(
        `On-the-way text sent to ${customer.first_name || "customer"}.`,
        "success"
      );
    } else if (textResult.duplicate === true) {
      showMessage(
        textResult.reason ||
          "An on-the-way text was already sent recently.",
        "info"
      );
    } else if (textResult.skipped === true) {
      showMessage(
        textResult.reason ||
          "SMS was not sent for this customer.",
        "info"
      );
    } else {
      showMessage(
        textResult.reason ||
          textResult.details ||
          "The stop was marked en route, but the text failed to send.",
        "warning"
      );
    }
  ...
  const handleComplete = async () => {
    ...
    if (updatedCustomer.sms_consent === true) {
      const textResult = await sendText(
        updatedCustomer,
        "completed"
      );
      ...
      if (textResult.sent === true) {
        ...
        showMessage(
          appliedDeodorizerNow
            ? "Service completed, deodorizer updated, and history saved. A duplicate completion text was prevented."
            : "Service completed and history saved. A duplicate completion text was prevented.",
          "info"
        );
      } else if (textResult.skipped === true) {
        showMessage(
          appliedDeodorizerNow
            ? "Service completed, deodorizer updated, and history saved. The customer notification was skipped."
            : "Service completed and history saved. The customer notification was skipped.",
          "info"
        );
      } else {
        showMessage(
          appliedDeodorizerNow
            ? "Service completed, deodorizer updated, and history saved, but the customer text failed to send."
            : "Service completed and history saved, but the customer text failed to send.",
          "warning"
        );
      }
    } else {
      showMessage(
        appliedDeodorizerNow
          ? "Service completed, deodorizer updated, and history saved. SMS is not enabled for this customer."
          : "Service completed and history saved. SMS is not enabled for this customer.",
        "success"
      );
    }
        ...
        <DialogContent dividers>
          {completionCustomer && (
            <Stack spacing={2}>
              ...
              <Typography>
                {completionCustomer.sms_consent === true
                  ? "Confirm the stop details before saving service history and sending the completion notification."
                  : "Confirm the stop details before saving service history. SMS is not enabled for this customer, so no text will be sent."}
              </Typography>

              {completionCustomer.sms_consent !== true && (
                <Alert severity="info">
                  This customer did not opt in to SMS notifications. The service and gate photo will still be saved normally.
                </Alert>
              )}
